module.exports = function(connection){
  ...
  this.getFeatured = function(cb){
      executeQuery('select featured.song_id , song.name as name,song.play_count,song.download_count,song.flame_count,song.src as url ,user.id as user_id,user.location,user.name as artist ,image_art.src as art,genre.genre from featured,genre,song,user, image_art where featured.song_id =song.id and song.genre_id = genre.id and song.user_id = user.id and image_art.song_id = song.id order by flame_count desc',cb);
  }
